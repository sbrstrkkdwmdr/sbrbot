...
module.exports = (userdata, client, Discord, osuApiKey, osuClientID, osuClientSecret, config) => {
    ...
    client.on('guildBanAdd', (ban) => {
        ...
        fs.appendFileSync(`./logs/${guild.id}.log`, `\nguildBanAdd event\n${currentDate} | ${currentDateISO}\n `);
        fs.appendFileSync(`./logs/${guild.id}.log`, `Guild Member ${ban.user.username}#${ban.user.discriminator} (${ban.user.id}) was banned`)//by ${ban.client.user.username}#${ban.client.user.discriminator} (${ban.client.user.id})\n
    })
    client.on('guildBanRemove', (ban) => {
        ...
        fs.appendFileSync(`./logs/${guild.id}.log`, `Guild Member ${ban.user.username}#${ban.user.discriminator} (${ban.user.id}) was unbanned\n`)//by ${ban.client.user.username}#${ban.client.user.discriminator} (${ban.client.user.id})
    })
    client.on('guildMemberRemove', (member) => {
        let currentDate = new Date();
        let currentDateISO = new Date().toISOString();
        let guild = client.guilds.cache.get(member.guild.id)
        fs.appendFileSync(`./logs/${guild.id}.log`, `guildMemberRemove event\n${currentDate} | ${currentDateISO}\n `);
        fs.appendFileSync(`./logs/${guild.id}.log`, `Guild Member ${member.user.username}#${member.user.discriminator} (${member.user.id}) left the server or was kicked\n`)
    })
    client.on('guildMemberUpdate', (oldMember, newMember) => {
        ...
        if (oldMember.user.username != newMember.user.username) {
            fs.appendFileSync(`./logs/${guild.id}.log`, `username changed: ${oldMember.user.username} => ${newMember.user.username}\n`)
        }
        if (oldMember.displayName != newMember.displayName) {
            fs.appendFileSync(`./logs/${guild.id}.log`, `display name changed: ${oldMember.displayName} => ${newMember.displayName}\n`)
        }
        if (oldMember.roles.cache != newMember.roles.cache) {
            fs.appendFileSync(`./logs/${guild.id}.log`, `roles changed: ${oldMember.roles.cache.map(r => r.name).join(', ')} => ${newMember.roles.cache.map(r => r.name).join(', ')}\n`)
        }
    })
    ...
    client.on('userUpdate', (oldUser, newUser) => {
        ...
        client.guilds.cache.forEach(guild => {
            if (guild.members.cache.has(oldUser.id)) {
                fs.appendFileSync(`./logs/${guild.id}.log`, `\nuserUpdate event\n${currentDate} | ${currentDateISO}\n `);
                fs.appendFileSync(`./logs/${guild.id}.log`, `User ${oldUser.username}#${oldUser.discriminator} (${oldUser.id}) has been updated:\n`)
                if (oldUser.avatar != newUser.avatar) {
                    fs.appendFileSync(`./logs/${guild.id}.log`, `avatar changed: ${oldUser.avatarURL()} => ${newUser.avatarURL()}\n`)
                }
                if (oldUser.username != newUser.username) {
                    fs.appendFileSync(`./logs/${guild.id}.log`, `username changed: ${oldUser.username} => ${newUser.username}\n`)
                }
                if (oldUser.discriminator != newUser.discriminator) {
                    fs.appendFileSync(`./logs/${guild.id}.log`, `discriminator changed: ${oldUser.discriminator} => ${newUser.discriminator}\n`)
                }
    ...
    client.on('channelCreate', (channel) => {
        ...
        switch (true) {
            ...
            case channel.type == 2 || channel.type.toString() == 'GUILD_VOICE':
                fs.appendFileSync(`./logs/${guild.id}.log`, ` Voice Channel ${channel.name} (${channel.id}) was created\n`)
                break;
    ...
    client.on('channelDelete', (channel) => {
        ...
        switch (true) {
            case channel.type == 0 || channel.type.toString() == 'GUILD_TEXT':
                fs.appendFileSync(`./logs/${guild.id}.log`, ` Text Channel ${channel.name} (${channel.id}) was deleted\n`)
                break;
            case channel.type.toString() == 'GUILD_CATEGORY' || channel.type == 4:
                fs.appendFileSync(`./logs/${guild.id}.log`, ` Category ${channel.name} (${channel.id}) was deleted\n`)
                break;
            case channel.type == 2 || channel.type.toString() == 'GUILD_VOICE':
                fs.appendFileSync(`./logs/${guild.id}.log`, ` Voice Channel ${channel.name} (${channel.id}) was deleted\n`)
                break;
            case channel.type == 5 || channel.type.toString() == 'GUILD_NEWS':
    ...
    client.on('channelUpdate', (oldChannel, newChannel) => {
        ...
        fs.appendFileSync(`./logs/${guild.id}.log`, `\nchannelUpdate event\n${currentDate} | ${currentDateISO}\n `);
        fs.appendFileSync(`./logs/${guild.id}.log`, `Channel ${oldChannel.name} (${oldChannel.id}) was updated \n`)

        if (oldChannel.name != newChannel.name) {
            fs.appendFileSync(`./logs/${guild.id}.log`, `name changed: ${oldChannel.name} => ${newChannel.name}\n`)
        }
        if (oldChannel.parent != newChannel.parent) {
            fs.appendFileSync(`./logs/${guild.id}.log`, `parent changed: ${oldChannel.parent.name} => ${newChannel.parent.name}\n`)
        }
        if (oldChannel.permissionOverwrites != newChannel.permissionOverwrites) {
            fs.appendFileSync(`./logs/${guild.id}.log`, `permission overwrites changed: \nALLOW: ${oldChannel.permissionOverwrites.cache.map(p => p.allow.toArray().join(', '))} \n DENY: ${oldChannel.permissionOverwrites.cache.map(p => p.deny.toArray().join(', '))} \n=> \nALLOW: ${newChannel.permissionOverwrites.cache.map(p => p.allow.toArray().join(', '))} \n DENY: ${newChannel.permissionOverwrites.cache.map(p => p.deny.toArray().join(', '))}\n`)
        }
        if (oldChannel.topic != newChannel.topic) {
            fs.appendFileSync(`./logs/${guild.id}.log`, `topic changed: ${oldChannel.topic} => ${newChannel.topic}\n`)
        }
    ...
    client.on('emojiCreate', (emoji) => {
        ...
        fs.appendFileSync(`./logs/${guild.id}.log`, `\nemojiCreate event\n${currentDate} | ${currentDateISO}\n `);
        fs.appendFileSync(`./logs/${guild.id}.log`, `Emoji ${emoji.name} (${emoji.id}) was created => ${emoji.url}\n`)
    })
    ...
    client.on('emojiUpdate', (oldEmoji, newEmoji) => {
        ...
        if (oldEmoji.name != newEmoji.name) {
            fs.appendFileSync(`./logs/${guild.id}.log`, `name changed: ${oldEmoji.name} => ${newEmoji.name}\n`)
        }
        if (oldEmoji.identifier != newEmoji.identifier) {
            fs.appendFileSync(`./logs/${guild.id}.log`, `identifier changed: ${oldEmoji.identifier} => ${newEmoji.identifier}\n`)
        }
    ...
    client.on('guildUnavailable', (guild) => {
        ...
        fs.appendFileSync(`./logs/${guildlog.id}.log`, `\nguildUnavailable event\n${currentDate} | ${currentDateISO}\n `);
        fs.appendFileSync(`./logs/${guildlog.id}.log`, `Guild ${guild.name} (${guild.id}) was unavailable\n`)
    })
    client.on('guildUpdate', (oldGuild, newGuild) => {
        ...
        fs.appendFileSync(`./logs/${guild.id}.log`, `\nguildUpdate event\n${currentDate} | ${currentDateISO}\n `);
        fs.appendFileSync(`./logs/${guild.id}.log`, `Guild ${oldGuild.name} (${oldGuild.id}) was updated\n`)
    ...
    client.on('inviteCreate', (invite) => {
        ...
        fs.appendFileSync(`./logs/${guild.id}.log`, `\ninviteCreate event\n${currentDate} | ${currentDateISO}\n `);
        fs.appendFileSync(`./logs/${guild.id}.log`, `Invite ${invite.code} was created (${invite.url})\n`)
    })
    client.on('inviteDelete', (invite) => {
        ...
        fs.appendFileSync(`./logs/${guild.id}.log`, `Invite ${invite.code} was deleted (${invite.url})\n`)
    })
    client.on('roleCreate', (role) => {
        ...
        fs.appendFileSync(`./logs/${guild.id}.log`, `\nroleCreate event\n${currentDate} | ${currentDateISO}\n `);
        fs.appendFileSync(`./logs/${guild.id}.log`, `Role ${role.name} (${role.id}) was created\n`)
    })
    client.on('roleDelete', (role) => {
        ...
        let guild = client.guilds.cache.get(role.guild.id)
        fs.appendFileSync(`./logs/${guild.id}.log`, `\nroleDelete event\n${currentDate} | ${currentDateISO}\n `);
        fs.appendFileSync(`./logs/${guild.id}.log`, `Role ${role.name} (${role.id}) was deleted\n`)
    })
    client.on('roleUpdate', (oldRole, newRole) => {
        ...
        if(oldRole.name != newRole.name) {
            fs.appendFileSync(`./logs/${guild.id}.log`, `name changed: ${oldRole.name} => ${newRole.name}\n`)
        }
        if(oldRole.color != newRole.color) {
            fs.appendFileSync(`./logs/${guild.id}.log`, `colour changed: ${oldRole.color} => ${newRole.color}\n`)
        }
        if(oldRole.permissions != newRole.permissions) {
            fs.appendFileSync(`./logs/${guild.id}.log`, `permissions changed: ${oldRole.permissions.toArray().join(', ')} => ${newRole.permissions.toArray().join(', ')}\n`)
        }
    })
    client.on('stageInstanceCreate', (stageInstance) => {
        ...
        fs.appendFileSync(`./logs/${guild.id}.log`, `Stage Instance ${stageInstance.topic} (${stageInstance.id}) was created in ${stageInstance.channel} (${stageInstance.channel.id})\n`)
    })
    client.on('stageInstanceDelete', (stageInstance) => {
        ...
        fs.appendFileSync(`./logs/${guild.id}.log`, `Stage Instance ${stageInstance.topic} (${stageInstance.id}) was deleted in ${stageInstance.channel} (${stageInstance.channel.id})\n`)
    })
    client.on('stageInstanceUpdate', (oldStageInstance, newStageInstance) => {
        ...
        fs.appendFileSync(`./logs/${guild.id}.log`, `Stage Instance ${oldStageInstance.topic} (${oldStageInstance.id}) was updated in ${oldStageInstance.channel} (${oldStageInstance.channel.id})\n`)
        if (oldStageInstance.topic != newStageInstance.topic) {
            fs.appendFileSync(`./logs/${guild.id}.log`, `topic changed: ${oldStageInstance.topic} => ${newStageInstance.topic}\n`)
        }
        if (oldStageInstance.channel != newStageInstance.channel) {
            fs.appendFileSync(`./logs/${guild.id}.log`, `channel changed: ${oldStageInstance.channel} (${oldStageInstance.channel.id}) => ${newStageInstance.channel} (${newStageInstance.channel.id})\n`)
        }
        if (oldStageInstance.privacyLevel != newStageInstance.privacyLevel) {
            fs.appendFileSync(`./logs/${guild.id}.log`, `privacy level changed: ${oldStageInstance.privacyLevel} => ${newStageInstance.privacyLevel}\n`)
        }
