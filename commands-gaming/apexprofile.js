...
module.exports = {
    ...
    execute(message, args, client, Discord, currentDate, currentDateISO, trnkey) {
        ...
        const profileurl =  `https://public-api.tracker.gg/v2/apex/standard/profile/${platform}/${platformUserIdentifier} `
        ...
        .then(profileoutput => {
            fs.writeFileSync("apexprofile.json", JSON.stringify(profileoutput, null, 2))
            ...
            let winningKills = JSON.stringify(profileoutput['data']['segments'][0]['stats']['wins'], ['value']).replaceAll('{', '').replaceAll('"', '').replaceAll('}', '').replaceAll(':', '').replaceAll('value', '');
            let revives = JSON.stringify(profileoutput['data']['segments'][0]['stats']['revives'], ['value']).replaceAll('{', '').replaceAll('"', '').replaceAll('}', '').replaceAll(':', '').replaceAll('value', '');
            let snipekill = JSON.stringify(profileoutput['data']['segments'][0]['stats']['sniperKills'], ['value']).replaceAll('{', '').replaceAll('"', '').replaceAll('}', '').replaceAll(':', '').replaceAll('value', '');
            ...
            .setImage(avatar)
            .setTitle(`${username}'s Apex Legends Profile`)
            .addField('PROFILE INFO', `Country: ${country} ${country.toLowerCase()}\n`, false)
            .addField('PROFILE STATS', `**Kills**:${kills} **Damage dealt**:${totaldmg} | **Matches played**: ${totalmatches}\n**Winning kills**:${winningKills} | **Revives**: ${revives}\n**Sniper kills**: ${snipekill}`, false)
