  ...
  execute(interaction, args, Discord, options, currentDate, currentDateISO) {
    ...
    switch (mathtype) {
      ...
      case "!":
        ...
        break;
      case "hcf":
      case "highestcommonfactor":
        if (isNaN(part1) || isNaN(part2))
          return interaction.reply("Error - NaN");
        if (part1 != Math.round(part1) || part2 != Math.round(part2))
          return interaction.reply("Error - numbers must be integers");
        if (part1 < 1 || part2 < 1)
          return interaction.reply("Error - values are too low");
        let problemhcf = findHCF(part1, part2);
        interaction.reply(`${problemhcf}`);
        break;
      case "lcm":
      case "lowestcommonmultiple":
        if (isNaN(part1) || isNaN(part2))
          return interaction.reply("Error - NaN");
        if (part1 != Math.round(part1) || part2 != Math.round(part2))
          return interaction.reply("Error - numbers must be integers");
