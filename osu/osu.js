    ...
    fetch(userinfourl, {
      ...
      .then((output2) => {
        try {
          ...
          let playerrank = playerrank1
            .toString()
            .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
          let playercountryrank = playercountryrank1
            .toString()
            .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
          ...
          let lastvismonths =
            Math.trunc(minlastvisreform / 60 / 24 / 30.437) % 12;
          ...
          if ((lastvismonths = 0)) {
            minlastvisredo =
              lastvisdays + "d | " + lastvishours + "h " + lastvisminutes + "m";
