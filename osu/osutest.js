...
module.exports = {
    ...
    execute(message, args, Discord, currentDate, currentDateISO, osuapikey, osuauthtoken, osuclientid, osuclientsecret) {
        if(message.author.id == '503794887318044675'){ 
            ...
            console.log("") 
                try{
                    const userfromfilereader = fs.readFileSync('scratch/user_ign')
                    const userfromfilereader2 = userfromfilereader.toString();
                    const userfromfile = JSON.stringify(userfromfilereader2).replaceAll('"', '').replaceAll("\\", '')
                    message.reply(`${userfromfile} ${userfromfilereader}`)
                    ...
                    const userinfourl = `https://osu.ppy.sh/api/v2/users/${userfromfile}/osu`;
