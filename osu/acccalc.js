...
module.exports = {
  ...
  async execute(
    ...
    let topequation = Math.floor(
      300 * hit300 + 100 * hit100 + 50 * hit50 + miss
    );
    ...
    (async () => {
      if (
        ...
        fs.appendFileSync(osulogdir, "\n" + "osu");
        topequation = Math.floor(
          300 * hit300 + 100 * hit100 + 50 * hit50 + miss
        );
        bottomequation = Math.floor(300 * (hit300 + hit100 + hit50 + miss));
        fullequation =
          Math.abs((topequation / bottomequation) * 100).toString() + "%";
        shortequation =
          Math.abs((topequation / bottomequation) * 100)
            .toFixed(2)
            .toString() + "%";
        ...
        if (
          (hit300 / totalhits > 0.7 && miss == 0) ||
          hit300 / totalhits > 0.8
        ) {
          grade = "B";
        }
        if (
          (hit300 / totalhits > 0.8 && miss == 0) ||
          hit300 / totalhits > 0.9
        ) {
          grade = "A";
        }
        if (
          Math.abs(hit300 / totalhits) > 0.9 &&
          miss == 0 &&
          Math.abs(hit50 / totalhits) < 0.01
        ) {
          grade = "S";
        }
        ...
        interaction.channel.send({
          content: `**Accuracy:** ${shortequation}\n**Full ver:** ${fullequation}\n**Grade:** ${grade}`,
        }); //can't use edit reply bcs INTERACTION_NOT_REPLIED error, and cant use reply bcs DiscordAPIError: Interaction has already been acknowledged
        fs.appendFileSync(osulogdir, "\n" + "");
        fs.appendFileSync(osulogdir, "\n" + "sent");
        console.groupEnd();
      }
      ...
      if (mode == "mania" || mode == "m" || mode == "4") {
        fs.appendFileSync(osulogdir, "\n" + "mania");
        topequation = Math.floor(
          300 * (hit300max + hit300) + 200 * hit200 + 100 * hit100 + 50 * hit50
        );
        bottomequation = Math.floor(
          300 * (hit300max + hit300 + hit200 + hit100 + hit50 + miss)
        );
        fullequation =
          Math.abs((topequation / bottomequation) * 100).toString() + "%";
        shortequation =
