        ...
        .then((output1) => {
          ...
          fetch(mapscoreurl, {
            ...
            .then((output2) => {
              ...
              for (i = 0; i < 5; i++) {
                let name =
                  output2.scores[i + pageoffset].user.username.toString();
                let pp = parseFloat(
                  output2.scores[i + pageoffset].pp.toString()
                ).toFixed(2);
                let score = output2.scores[i + pageoffset].score
                  .toString()
                  .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
                let acc = Math.abs(
                  output2.scores[i + pageoffset].accuracy.toString() * 100
                ).toFixed(2);
                let mods = output2.scores[i + pageoffset].mods
                  .toString()
                  .replaceAll(",", "")
                  .replaceAll("[", "")
                  .replaceAll("]", "");
                let date1 = output2.scores[i + pageoffset].created_at
                  .toString()
                  .slice(0, 10);
                let datetime = output2.scores[i + pageoffset].created_at
                  .toString()
                  .slice(11, 19);
                ...
                let hitmax =
                  output2.scores[
                    i + pageoffset
                  ].statistics.count_geki.toString();
                let hit300 =
                  output2.scores[
                    i + pageoffset
                  ].statistics.count_300.toString();
                let hit200 =
                  output2.scores[
                    i + pageoffset
                  ].statistics.count_katu.toString();
                let hit100 =
                  output2.scores[
                    i + pageoffset
                  ].statistics.count_100.toString();
                let hit50 =
                  output2.scores[i + pageoffset].statistics.count_50.toString();
                let miss =
                  output2.scores[
                    i + pageoffset
                  ].statistics.count_miss.toString();
