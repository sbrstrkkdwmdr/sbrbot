...
module.exports = (
  ...
  const eventstufffiles = fs
    .readdirSync("./event-stuff/")
          ...
          if (member.presence || member.presences) {
            ...
            for (i = 0; i < member.presence.activities.length; i++) {
              let activitytimestamp =
                member.presence.activities[i].createdTimestamp; //(new Date(member.presence.activities.createdAt)).getTime()
              ...
              if (
                member.presence.activities[i].name == "League of Legends" &&
                totaltimes > 1800
              ) {
                fs.appendFileSync(
                  "logs/presence.log",
                  `\n${member.id} AKA ${
                    member.username
                  } has been playing league for ${totaltimes / 60} minutes!\n`
                ); // saves it in logs
  ...
  client.on("messageCreate", (message) => {
    ...
    const foundantitriggers = antitriggers.find((v) =>
      triggerstring.includes(v)
    );

    if (
      foundtriggers &&
      message.channel != loggingchannel &&
      !foundantitriggers
    ) {
      client.eventstuff
        .get("triggers")
  ...
  client.on("messageUpdate", (oldMessage, newMessage) => {
    ...
    if (
      foundtriggers &&
      newMessage.channel != loggingchannel &&
      !foundantitriggers
    ) {
      client.eventstuff
        .get("triggers")
  ...
  client.on("presenceUpdate", (oldPresence, newPresence) => {
    ...
    if (newPresence.status != "offline") {
      ...
      for (i = 0; i < newPresence.activities.length; i++) {
        ...
        if (foundtriggers) {
          logchannel.send(
            `${newpresencetxt} - ${newPresence.userId} | <@${newPresence.userId}> presence warning\n guild - ${newPresence.guild.id} | ${newPresence.guild.name}`
          );
        }
      }
    }
