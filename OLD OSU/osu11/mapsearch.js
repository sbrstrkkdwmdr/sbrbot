...
module.exports = {
    ...
    execute(message, args, Discord, currentDate, currentDateISO, osuapikey, osuauthtoken, osuclientid, osuclientsecret) {
        ...
        pickeduserX1 = args.splice(0,1000).join(" ");
        pickeduserX = pickeduserX1.replaceAll(' ', '%20')
        //fs.appendFileSync(osulogdir, "\n" + args[1])

        fs.appendFileSync(osulogdir, "\n" + `${currentDateISO} | ${currentDate}`)
        fs.appendFileSync(osulogdir, "\n" + "command executed - map get (filename)")
        ...
        if(pickeduserX){
            ...
            mapurlold = `https://osu.ppy.sh/api/get_beatmaps?k=${osuapikey}&filename=${pickeduserX}`
            mapurl = `https://osu.ppy.sh/api/v2/beatmaps/lookup?k=${osuapikey}&filename=${pickeduserX}.osu`;
            //mapurl = `https://osu.ppy.sh/api/v2/beatmaps/lookup?k=${osuapikey}&checksum=${maphash}`
        }
        //mapurl = "https://osu.ppy.sh/api/v2/beatmaps/lookup?filename=Serizawa Yu - Devi-kyuu (TV Size) (- Ex -) [Kuroame's Insane].osu"
        console.log(mapurl)
        moddetect = 'NM'
        ...
        .then(outputold => {
            mapidold = JSON.stringify(outputold[0], ['beatmap_id']).replaceAll('{', '').replaceAll('"', '').replaceAll('}', '').replaceAll(':', '').replaceAll('beatmap_id', '');
        mapurl = `https://osu.ppy.sh/api/v2/beatmaps/${mapidold}`
        try{
