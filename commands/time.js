...
module.exports = {
  ...
  execute(message, args, Discord, currentDate, currentDateISO) {
    ...
    let thedaysthingyiuseonmydiscordstatus = (
      msepochsince /
      1000 /
      60 /
      60 /
     ...
     */
    function dayhuman(weekdaynum) {
      //date.getUTCDay returns an int so this is to convert to its name
    ...
    function tomonthname(monthnum) {
      //date.getUTCMonth returns an int so this is to convert to its name
    ...
    if (relmonthnum < 10) {
      relmonthnum = "0" + relmonthnum;
    }
    if (reldaynum < 10) {
      reldaynum = "0" + reldaynum;
    }
    ...
    if (args[0]) {
      if (!args[0].includes("/")) {
        ...
        let optionaldateoffset = fixoffset(
          new Date(optionaldateISO).getTimezoneOffset()
        );

        //let reldatenow12h = `${relday}, ${reldate} ${relmonth} ${relyear} ${reldatenow12hhours}`
        let optionaldate2 = `${dayhuman(optionaldate.getDay())}, ${tomonthname(
          optionaldate.getMonth()
        )} ${optionaldate.getDate()}`;
